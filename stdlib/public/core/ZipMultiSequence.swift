...
extension ZipMultiSequence {
  ...
  public struct Iterator: IteratorProtocol {
    ...
    /// `ZipMultiSequence`.
    private let _chainIterator: (repeat AnyIterator<(each Chain).Element>)
    ...
    fileprivate init(iterator: (repeat AnyIterator<(each Chain).Element>)) {
...
@available(macOS 14.0, *)
private extension ZipMultiSequence {
  /// Thrown when an underlying iterator's `next()` returns nil during value
  /// pack expansion in `ZipMultiSequence.Iterator.next()`.
  fileprivate struct _NextIsNil: Error {}
  ...
  fileprivate static func _someOrThrow<Wrapped>(
    _ wrapped: Wrapped?
  ) throws -> Wrapped {
